...
void Map::listDel(int z, int x) {
	...
	else map[map[z][x].prev.first][map[z][x].prev.second].next = POINTNULL;
	if (lastObj == p) lastObj = map[z][x].prev;
	else map[map[z][x].next.first][map[z][x].next.second].prev = POINTNULL;
}
